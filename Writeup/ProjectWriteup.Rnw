...
\begin{theorem}
Assume assumption 2. Then for an $N \geq 1$, the PIMH update is a standard IMH update on the extended space ${\bf X}$ with target density $\tilde{\pi}^N$ and proposal $q^N$.
\end{theorem}


\begin{theorem}
Assume assumptions 1 and 2. Then 
\begin{enumerate}
  \item the PIMH sampler generates a sequence $\{ X_{1:P}(i) \}$ whose marginal distributions $\{ \mathcal{L}^N \{ X_{1:P}(i) \in \cdot \} \}$ satisfy 
    \begin{equation*}
    \| \mathcal{L}^N \{ X_{1:P}(i) \in \cdot \} - \pi(\cdot) \| \rightarrow 0 \quad \quad \text{as }i \rightarrow \inf
    \end{equation*}
  %
  \item if assumption 3 holds, then there exists $\rho_P \in [0,1)$ s.t. $\forall i \geq 1$ and $x_{1:P} \in \mathcal{X}^P$ 
    \begin{equation*}
    \| \mathcal{L}^N \{ X_{1:P}(i) \in \cdot | X_{1:P}(0)=x_{1:P} \} - \pi(\cdot) \| \leq \rho^i_P
    \end{equation*}
\end{enumerate}
\end{theorem}
...
The following thereom shows the invariance of $p_\theta(x_{1:T}|y_{1:T})$ after PMMH update and erogodicity of PMMH.
\begin{theorem}
...
\begin{enumerate}
  ...
  %
  \item additionally, if assumptions 5 and 6 are met, the PMMH sampler generates a sequence $\{ \theta(i), X_{1:P}(i) \}$ whose marginal distributions $\{ \mathcal{L}^N \{ \theta(i), X_{1:P}(i) \in \cdot \} \}$ satisfy 
    \begin{equation*}
    \| \mathcal{L}^N \{ \theta(i), X_{1:P}(i) \in \cdot \} - \pi(\cdot) \| \rightarrow 0 \quad \quad \text{as }i \rightarrow \inf
    \end{equation*}
\end{enumerate}
\end{theorem}
...
This theorem shows invariance of $p_\theta(x_{1:T}|y_{1:T})$ holds after a Particle Gibbs update and is erogodic.
\begin{theorem}
Assume assumption 2. Then
\begin{enumerate}
  ...
  %
  \item if assumption 5-7 hold, the PG sampler generates a sequence $\{ \theta(i), X_{1:P}(i) \}$ whose marginal distributions $\{ \mathcal{L}^N \{ (\theta(i), X_{1:P}(i)) \in \cdot \} \}$ satisfy 
    \begin{equation*}
    \| \mathcal{L}^N \{ (\theta(i), X_{1:P}(i)) \in \cdot \} - \pi(\cdot) \| \rightarrow 0 \quad \quad \text{as }i \rightarrow \inf \quad \exists \quad N \geq 2
    \end{equation*}
\end{enumerate}
\end{theorem}
